...
|-----------|-----------|-------------------------------|-------|
| **P-01** | Data residency / sovereignty : keep regulated data in-country | Every service takes its location from `region`, chosen at deploy time and validated against the `allowed_regions` residency allowlist (default `asia-southeast1`; extending that list is the review point); regional Document AI / Model Armor endpoints + VPC-SC perimeter | `config/settings.yaml` (`region`), `Settings.region`, `infra/terraform/`, `docs/runbook.md` |
...
| **R5** | Pass the **Hrz4 eval gate** before promotion | Promotion blocked unless `EvalReport.passed`; `RemoteEvaluationAdapter` consumes Hrz4 `/v1/evaluations`; offline gate enforces it in CI | `EvaluationGatePort`, `adapters/platform/remote_evaluation.py`, `eval/run_eval.py`, `.github/workflows/eval-gate.yaml`, SPEC §6 Hrz4 |
...
| Eval gate blocks promotion (P-08, R5) | `make eval` exits non-zero on failure; `.github/workflows/eval-gate.yaml` |
