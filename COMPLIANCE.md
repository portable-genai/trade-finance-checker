...
| **R5** | Pass the **Hrz4 eval gate** before promotion | Promotion blocked unless `EvalReport.passed`; `RemoteEvaluationAdapter` consumes Hrz4 `/v1/evaluations`; offline gate enforces it in CI | `EvaluationGatePort`, `adapters/platform/remote_evaluation.py`, `eval/run_eval.py`, the hosted GitHub Actions check, SPEC §6 Hrz4 |
