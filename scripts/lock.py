#!/usr/bin/env python3
"""Compile every lockfile and put the header back, because `uv pip compile` overwrites it.
...
it. Run it after any dependency change and commit every file it rewrites.
"""
...
#: (extra, output file) for EVERY pyproject-derived lockfile the repo ships, and "every" is
#: load-bearing. `requirements-demo.lock` pins the headless-browser extra the served-browser
#: evidence installs ON TOP of the dev lock. A lockfile left out of this tuple is regenerated by
#: hand or not at all, so it sits on a superseded commons pin and DOWNGRADES a kit the dev lock
#: had just installed correctly, with no diff anywhere to notice.
_TARGETS = (
...
# `pyproject.toml` names and against every other lockfile:
{tag_map}
"""
