"""ReviewRouterPort: the boundary that routes an escalated discrepancy report to Hrz7 (rule R8).
...
class ReviewRouterPort(Protocol):
    def route(self, report: DiscrepancyReport, *, maker: str, tenant: str = "") -> None:
        """Route an escalated report to Hrz7 for human review (idempotent per LC is ideal)."""
        ...
