...
- **Demo C - YOUR presentation under the `live` profile** (the audience-facing demo):
  ...
  comes from the deterministic detector, and the report prose is generated by the fleet's
  shared local Gemma model through `hex_service_kit.localmodel`. Rule citations reference the real UCP600 article numbers with the
...
```bash
# 1. Start the shared local model server on :8001 (LOCAL_MODEL_URL / LOCAL_MODEL override it).
