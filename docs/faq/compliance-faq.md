...
HIGH/CRITICAL discrepancies escalate to enhanced review and are routed to the Hrz7
...
guardrail/DLP itself is the sibling **Hrz1** gateway; this repo consumes it.
...
1.0. The enterprise promotion gate and red-team harness are the sibling **Hrz4** system; this
repo's gate mirrors its thresholds so merges are guarded locally, and gate mode refuses to run
outside `TRADE_FINANCE_PROFILE=platform|gcp`, where the Hrz4 promotion gate owns the decision.
...
Doc4-control column is stable across regulators. At scale, **Rsk1** `compliance-advisory` and
